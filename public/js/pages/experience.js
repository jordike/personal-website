    ...
    <div class="accordion-item m-3">
        ...
        <div id="function-%functionIndex%-target" class="accordion-collapse collapse m-3" data-bs-parent="#functions">
            <div class="accordion-body p-2">
                <input type="hidden" name="Functions[%functionIndex%].Id" value="%functionId%" />
                <div class="form-group">
                    <label class="form-label required-input">Functie</label>
                    <input id="function-title-input-%functionIndex%" class="form-control" type="text" name="Functions[%functionIndex%].FunctionTitle" value="%functionTitle%" onchange="updateFunctionTitle(%functionIndex%)" %disabled% />
                </div>
                <div class="form-group">
                    <label class="form-label">Omschrijving</label>
                    <textarea class="form-control" type="text" name="Functions[%functionIndex%].Description" value="%functionDescription%" %disabled%></textarea>
                </div>
                <div class="form-group">
                    <label class="form-label required-input">Startdatum</label>
                    <input class="form-control" type="date" name="Functions[%functionIndex%].StartDate" value="%functionStartDate%" %disabled% />
                </div>
                <div class="form-group">
                    <label class="form-label">Einddatum</label>
                    <input class="form-control" type="date" name="Functions[%functionIndex%].EndDate" value="%functionEndDate%" %disabled% />
                </div>
            </div>
        </div>
    </div>
...
function addNewFunction(data, editable) {
    ...
    if (typeof data === "undefined") {
        html = html.replace(/%functionId%/g, "-1");
        html = html.replace(/%functionTitle%/g, "Nieuwe functie");
    } else {
        replaceMarker("functionId");
        replaceMarker("functionTitle");
    }

    replaceMarker("functionStartDate");
    replaceMarker("functionEndDate");
    ...
    if (typeof data !== "undefined") {
        ...
        if (description !== null) {
            description.textContent = data["functionDescription"];
        }
    }
